...
def test_sample(net, val_loader, logger, log_dir, summary_writer, args, cfg):
    ...
    avg_time = time_total/len(val_loader)
    logger.info(f'\nTest on {len(val_loader)} instances\n {total_err_metrics}\n avg_infer_time {avg_time}\n')

    # Save object error to csv file
    print(total_obj_depth_err,total_obj_count,total_obj_depth_err / total_obj_count)
